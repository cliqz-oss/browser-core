...
export default class UIWindow extends AppWindow {
  ...
  */
  init() {
    ...
    return autocompleteLoadingPromise.then(() => this.dropdown.windowAction(this.window, 'init'))
      .then(() => {
  ...
  autocompleteQuery(firstResult, firstTitle) {
    ...
    const lastPattern = autocomplete.lastPattern;
    const fRes = lastPattern ? lastPattern.filteredResults() : null;
    ...
    const results = lastPattern ? fRes : [];
    ...
    // Detect autocomplete
    const historyClusterAutocomplete = autocomplete.CliqzHistoryCluster
      .autocompleteTerm(urlBar.mInputField.value, results[0], true);
    ...
    urlBar.setSelectionRange(historyClusterAutocomplete.selectionStart,
