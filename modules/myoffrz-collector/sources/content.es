...
const MODULE_NAME = 'myoffrz-collector';
...
registerContentScript(MODULE_NAME, 'http*', (window, chrome, CLIQZ) => {
  if (window.top === window) { // We need only top window (no iframes)
    ...
    const categoryCollector = new CategoryCollector(window, backgroundModule);
    const callback = categoryCollector.onBackgroundMsg.bind(categoryCollector);
