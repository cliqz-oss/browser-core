...
const MODULE_NAME = 'regex_helper';
...
export default class RegexpHelper {
  ...
  buildUrlMatchData(urlData) {
    if (!urlData || !urlData.domain || !urlData.url) {
      lwarn(`buildUrlMatchData: invalid url data argument: ${JSON.stringify(urlData)}`);
      return {};
    }
    ...
    const numChars = Math.min(urlData.url.length, MAX_URL_LENGTH);

    for (let i = 0; i < numChars; i += 1) {
      const ch = urlData.url[i];
      let il = indexMap[ch];
      if (!il) {
        il = indexMap[ch] = [];
      }
  ...
  compileRegexObject(regexData) {
    if (!regexData || regexData.length === 0) {
      lwarn('compileRegexObject: empty regexData?');
      return {};
    }
    function compileAndAddRegexMap(aRegMap, regList, regStr) {
      ...
      if (!buildRegIdx) {
        ...
        try {
          ...
          lerr(`compileAndAddRegexMap: error building regex for ${regStr}`);
        }
        if (!regExpObj) {
          return -1;
        }
