...
const MODULE_NAME = 'persistent_cache_db';
...
export default class PersistentCacheDB {
  ...
  saveEntries(force = false) {
    if (!this.db || !this.docName) {
      lwarn('saveEntries: no db set or no doc set?');
  ...
  loadEntries(force = false) {
    if (!this.db || !this.docName) {
      lwarn('loadEntries: no db set or no doc set?');
      return Promise.resolve(false);
    }
    if (!force && !this.configs.should_persist) {
      // nothing to do
    ...
    return self.db.getDocData(this.docName).then((docData) => {
      if (!docData || !docData.entries) {
        lerr('loadEntries: something went wrong loading the data?');
        return Promise.resolve(false);
      }
      ...
      lerr(`loadEntries: error loading the storage data...: ${JSON.stringify(err)}`);
