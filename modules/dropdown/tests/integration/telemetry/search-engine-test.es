...
export default function () {
  ...
  describe('generates correct telemetry signals for a search engine result', function () {
    ...
    const enterSignalFields = [
      ...
      { name: 'query_length', type: 'number', expValue: 10 },
      { name: 'reaction_time', type: 'number' },
      { name: 'result_order', type: 'object', resultLength: 3 },
      { name: 'search', type: 'boolean', expValue: true },
      { name: 'v', type: 'number' },
    ];
    ...
    xcontext('after pressing Ctrl key and clicking on the left mouse button', function () {
      beforeEach(function () {
        ...
        const resultToClick = $cliqzResults.querySelector(resultSelector);
