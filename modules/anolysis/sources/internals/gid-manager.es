...
export default class GidManager {
  ...
  updateState() {
    if (this.isReadyPromise === null) {
      this.isReadyPromise =
        this.setLatestDemographics(JSON.stringify(this.demographics))
  ...
  handleNewInstall(demographics) {
    ...
    return this.backend.newInstall(demographics)
  ...
  handleReappearingUser(demographics) {
    ...
    return this.backend.reappearingUser(demographics)
      .then((formattedDemographics) => {
  ...
  handleUpdate(demographics) {
    ...
    return this.backend.updateGID(demographics)
      .then((gid) => {
  ...
  handleActiveUserSignal(demographics) {
    ...
    if (lastTimeDemographicsSent !== undefined) {
      ...
      if (currentDate.isSame(lastDemographicSentDate.add(6, 'months'), 'month')) {
        ...
        return this.backend.activeUserSignal(candidate)
          .then(formattedDemographics => Promise.all([
            this.setCurrentDemographics(formattedDemographics),
