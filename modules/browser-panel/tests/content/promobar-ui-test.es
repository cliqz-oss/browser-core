...
describe('Promo bar', function () {
  let subject;
  const buildUrl = '/build/cliqz@cliqz.com/chrome/content/browser-panel/index.html';
  ...
  Object.keys(sizes).forEach(function (frameWidth) {
    context(`rendered in a window ${frameWidth}px wide`, function () {
      before(function () {
        subject = new Subject();
        return subject.load(buildUrl, { iframeWidth: frameWidth })
          .then(function () {
            return subject.pushData(target, data, 'render_template');
          })
          .then(function () {
      ...
      context('the promo bar body', function () {
        ...
        if (sizes[frameWidth].hasFlags) {
          it('has a flags area with correct amount of flags', function () {
            expect($flags.length).to.equal(data.labels.length);
          });

          it('has a flags area with correct flags', function () {
            expect($flags.length).to.be.above(0);
            [...$flags].forEach(function (flag, i) {
              expect(flag).to.contain.text(data.labels[i]);
            });
          });
        }
