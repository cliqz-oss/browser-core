import {
  blurUrlBar,
  $cliqzResults,
  expect,
  fillIn,
  focusOnTab,
  mockSearch,
  newTab,
  press,
  testServer,
  testsEnabled,
  waitFor,
  waitForPopup,
  win,
  withHistory } from './helpers';

import { getResourceUrl } from '../../../tests/core/integration/helpers';

export default function () {
  if (!testsEnabled()) { return; }

  context('last query', function () {
    const freshtabUrl = getResourceUrl('freshtab/home.html');

    context('for "Search with"', function () {
      const url = 'https://url_test.com';
      const query = 'test';

      beforeEach(async function () {
        await testServer.registerPathHandler('/search', { result: '<html><body><p>success</p></body></html>' });

        win.CLIQZ.TestHelpers.searchEngines.addEngineWithDetails({
          key: '#te',
          url: testServer.getBaseUrl('search?q={searchTerms}'),
          name: 'Test',
          iconURL: 'data:image/png;base64,',
          method: 'GET',
          encoding: 'UTF-8',
        });

        await waitFor(function () {
          let isAdded = false;
          win.CLIQZ.TestHelpers.searchEngines.getSearchEngines().forEach((engine) => {
            if (engine.name === 'Test') {
              isAdded = true;
            }
          });
          return isAdded;
        });

        win.CLIQZ.TestHelpers.searchEngines.setDefaultSearchEngine('Test');

        await focusOnTab(await newTab(freshtabUrl));
        blurUrlBar();
        await mockSearch({ results: [{ url }] });
        withHistory([]);
        fillIn(query);
        await waitForPopup(2);
      });

      afterEach(async function () {
        win.CLIQZ.TestHelpers.searchEngines.setDefaultSearchEngine('Google');
        win.CLIQZ.TestHelpers.searchEngines.removeEngine('Test');
      });

      it('on Enter renders last query box with the query', async function () {
        press({ key: 'Enter' });
        const el = win.gURLBar.mInputField.parentElement;
        await waitFor(() => expect(el.querySelector('hbox.cliqz-urlbar-Last-search-container')).to.exist);
        expect(el.querySelector('hbox hbox.cliqz-urlbar-Last-search')).to.exist;
        expect(el.querySelector('.cliqz-urlbar-Last-search').textContent)
          .to.equal(query);
      });
    });

    context('for backend result without autocomplete', function () {
      const url = testServer.getBaseUrl('url-test');
      const query = 'test';

      beforeEach(async function () {
        await testServer.registerPathHandler('/url-test', { result: '<html><body><p>success</p></body></html>' });

        await focusOnTab(await newTab(freshtabUrl, true));

        blurUrlBar();
        await mockSearch({ results: [{ url }] });
        withHistory([]);
        fillIn(query);
        await waitForPopup(2);
      });

      it('on Enter renders last query box with the query', async function () {
        press({ key: 'ArrowDown' });
        await waitFor(() =>
          $cliqzResults.querySelector(`a.result[data-url="${url}"]`).classList.contains('selected'));
        press({ key: 'Enter' });
        const el = win.gURLBar.mInputField.parentElement;
        await waitFor(() => expect(el.querySelector('hbox.cliqz-urlbar-Last-search-container')).to.exist);
        expect(el.querySelector('hbox hbox.cliqz-urlbar-Last-search')).to.exist;
        expect(el.querySelector('.cliqz-urlbar-Last-search').textContent)
          .to.equal(query);
      });
    });

    context('for backend result with autocomplete', function () {
      const url1 = 'https://test_url1.com';
      const url2 = testServer.getBaseUrl('url-test');
      const query = 'local';

      beforeEach(async function () {
        await testServer.registerPathHandler('/url-test', { result: '<html><body><p>success</p></body></html>' });

        await focusOnTab(await newTab(freshtabUrl));

        blurUrlBar();
        await mockSearch({
          results: [
            { url: url1 },
            { url: url2 }
          ]
        });
        withHistory([]);
        fillIn(query);
        await waitForPopup(3);
      });

      it('on Enter doesn\'t render last query box', async function () {
        press({ key: 'ArrowDown' });
        await waitFor(() =>
          $cliqzResults.querySelector(`a.result[data-url="${url1}"]`).classList.contains('selected'));
        press({ key: 'ArrowDown' });
        await waitFor(() =>
          $cliqzResults.querySelector(`a.result[data-url="${url2}"]`).classList.contains('selected'));
        press({ key: 'Enter' });
        const el = win.gURLBar.mInputField.parentElement;
        await waitFor(() => expect(el.querySelector('hbox')).to.exist);
        expect(el.querySelector('hbox hbox.cliqz-urlbar-Last-search')).to.exist;
        expect(el.querySelector('hbox')).to.have.class('hidden');
      });
    });
  });
}
