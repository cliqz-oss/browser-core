    ...
    it('renders correct logo', async function () {
      ...
      if (isDefault) {
        await waitFor(async () => {
          ...
          return expect($logos[0].textContent).to.equal(imageName);
        }, 10000);
      } else {
        await waitFor(async () => {
          $logos = await getElements({
            elementSelector: '[aria-label="header-container"] [aria-label="generic-logo"] img',
            url,
          });
          expect($logos).to.have.length(1);
          return expect($logos[0].src).to.contain(`https://cdn.cliqz.com/brands-database/database/1521469421408/logos/${imageName}/$.svg`);
        }, 10000);
      }
    });
  });
