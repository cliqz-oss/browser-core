...
export default function () {
  context('for a weather forecast', function () {
    ...
    it('renders result with an existing and correct title', function () {
      const titleSelector = ".abstract span[class='title']";
    ...
    it('renders result with an existing logo', function () {
      const titleSelector = ".icons span[class='logo']";
      chai.expect(resultElement.querySelector(titleSelector)).to.exist;
    });
    ...
    it('renders result with a button link to extended forecast', function () {
      const forecastExtendedSelector = '.buttons a.btn';
      chai.expect(resultElement.querySelector(forecastExtendedSelector)).to.exist;
    });

    it('renders result with a button with correct link to extended forecast', function () {
      const forecastExtendedSelector = '.buttons a.btn';
      const forecastExtendedButton = resultElement.querySelector(forecastExtendedSelector);
      chai.expect(forecastExtendedButton.src).to.equal(results[0].snippet.deepResults[0].links.url);
    });
  });
}
