...
import results from './fixtures/resultsFlightDelayedEarlyDep';

export default function () {
  context('for flight results when flight has been delayed', function () {
    ...
    ['depart', 'arrival'].forEach(function (flight, i) {
      context(`for ${flight} info`, function () {
        ...
        it(`renders existing and correct ${flight} times`, function () {
          ...
          chai.expect(getComputedStyle(flightEstTimeItem).color).to.contain(grey);

          if (flight === 'depart') {
            chai.expect(getComputedStyle(flightActTimeItem).color).to.contain(green);
          } else {
            chai.expect(getComputedStyle(flightActTimeItem).color).to.contain(red);
          }

          chai.expect(flightEstTimeItem.className).to.contain('strike-through');
