...
export default function () {
  ...
  context('for soccer Europa League group results changing Group', function () {
    const locale = CliqzUtils.locale.default || CliqzUtils.locale[window.navigator.language];
    let $resultElement;
    ...
    before(function () {
      respondWith({ results });
      withHistory([]);
      fillIn('http://www.uefa.com/uefachampionsleague');
      return waitForPopup().then(function () {
        $resultElement = $cliqzResults()[0];
      }).then(function () {
    ...
    describe('renders a results table', function () {
      ...
      context('with a groups header with tabs', function () {
        ...
        it('with the second tab selected', function () {
          expect(soccerTableSecondTab.checked).to.equal(true);
        });

        it('with remaining tabs not selected', function () {
          expect(soccerTableFirstTab.checked).to.equal(false);
          [...soccerTableRemainingTabs].forEach(function (tab) {
            expect(tab.checked).to.equal(false);
          });
        });
